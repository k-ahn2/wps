...
| `channel_subscriptions` | array of channel ids | [`dbCreateNewUser`](/handlers.py#L317) (env defaults); [`channel_subscribe_handler`](/handlers.py#L1629) add / [remove](/handlers.py#L1653); [`check_auto_subscriptions`](/handlers.py#L1996) | Channels the user is subscribed to. WPS uses this to determine whether a user should receive a) updates when connected, b) push notifications when not connected (if enabled) |
