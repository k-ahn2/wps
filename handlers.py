...
# All message processing/business logic lives here (not in wps.py), so it can be warm-reloaded
# without disconnecting TCP connections.
#
# wps.py owns the TCP socket: accepting connections, reading/buffering bytes off the wire, and
# the per-connection lifecycle. It calls into this module (via `import handlers`, never
...
def send_push_notification(heading, message, player_id):
    ...
    try:
        ...
        # {'errors': {'invalid_player_ids': ['049b035b-c722-4b2b-b89e-f7ae6ee25835']}}
        if 'errors' in notificationResponse:
            raise Exception(notificationResponse.get('errors'))
        else:
...
def cleanup_bad_push_player_id(CONN_DB_CURSOR, callsign, bad_player_id, reason=None):
    ...
    try:
        ...
        if callsign_search['data'] == None:
            wps_logger("CLEANUP PUSH NOTIFICATION", callsign, "User not found when cleaning up bad player ID", 'ERROR')
            return

        user_record = callsign_search['data']
        user_push_records = user_record.get("push", [])

        def get_player_ids(user):
            return [p.get("playerId") for p in user.get("push", [])]

        wps_logger("CLEANUP PUSH NOTIFICATION", callsign, f"Push entries before: {get_player_ids(user_record)}")
        
        found_push_record = False
...
def existing_connect_handler(CONN_DB_CURSOR, callsign, connect_object, CONN, user_db_record, previous_connect_timestamp):
    ...
    if len(new_message_edits['data']) > 0:
        ...
        socket_send_handler(CONN_DB_CURSOR, CONN, callsign, message_edits_batch)
    ...
    if len(new_message_emojis['data']) > 0:
        ...
        socket_send_handler(CONN_DB_CURSOR, CONN, callsign, message_emojis_batch)
    ...
    wps_logger('CONNECT HANDLER', callsign, f"User updates response: {user_response.get('success', None)}")
...
def message_send_handler(CONN_DB_CURSOR, message, callsign, CONN):
    ...
    try:
        ...
        if len(push) > 0 and callsign not in notifications_since_last_logout:
            ...
            for p in push:
                if p['isPushEnabled'] and not p.get('isBadPlayerId'):
...
def message_edit_handler(CONN_DB_CURSOR, msg_update, callsign, CONN):
    ...
    socket_send_handler(CONN_DB_CURSOR, CONN, callsign, resp)
...
def post_handler(CONN_DB_CURSOR, post, callsign, CONN, suppress_cpr=False):
    '''
...
def post_edit_handler(CONN_DB_CURSOR, post_update, callsign, CONN):
    ...
    socket_send_handler(CONN_DB_CURSOR, CONN, callsign, client_acknowledgement)
...
def close_connection(CONN_DB_CURSOR, callsign, CONN):
    ...
    # that has the same callsign. Locate and remove it, and count any remaining
    # connections for this callsign, as a single atomic operation so the count
    # can't be skewed by a concurrent append/removal from another thread.
    ...
    wps_logger("DISCONNECT HANDLER", callsign, f"All connections AFTER disconnect: {[c['callsign'] for c in connections_snapshot()]}")
...
def socket_send_handler(CONN_DB_CURSOR, CONN, callsign, payload):
    ...
    event_logger(timestamp_milliseconds(), 'WPS_SEND', callsign, { "type": payload["t"], "bytes": len(send) }, None)
...
def process_message(CONN_DB_CURSOR, message_json, callsign, CONN):
    '''
    Dispatches one parsed JSON message from a client to the correct handler, keyed on its "t"
    type field. Called by wps.py's connected_session_handler for every complete packet read off
    the wire. Kept here (not in wps.py) so new message types and routing changes are picked up
    by a warm reload, same as every other handler.
    '''
    ...
    if message_json["t"] == "cp":
        ...
        post_handler(CONN_DB_CURSOR, message_json, callsign, CONN, suppress_cpr=(message_json.get("r") == 0))
